---
title: "Core Technology Stack"
description: "Specialized toolset for modern infrastructure automation across cloud platforms."
url: /stack/
layout: page
ShowToc: false
---

Specialised toolset for modern infrastructure automation.

| Domain | Technologies |
|---|---|
| **Container Orchestration** | Kubernetes (k3s, EKS, AKS, OKE), Kyverno, Helm, Kustomize |
| **Cloud Platforms** | AWS, GCP, Azure, Oracle Cloud (OCI) |
| **Infrastructure as Code** | Terraform / OpenTofu, Terragrunt, Packer, CloudFormation, ARM |
| **Configuration Management** | Ansible |
| **CI/CD & GitOps** | GitHub Actions, Jenkins, Azure DevOps, ArgoCD, Renovate |
| **Containers** | Docker, docker buildx (multi-arch), GitHub Container Registry |
| **Scripting & Automation** | Python, Bash, AWS Lambda |
| **Security** | Kyverno, Checkov, Trivy, AWS Config, Inspector |
| **Observability** | Prometheus, Grafana, PagerDuty, kube-prometheus-stack |
| **Storage** | Rook-Ceph, S3, Azure Storage |
| **Networking** | Transit Gateway, AWS WAF, API Management |
